<template>
    <div class="w-screen h-[1.5cm] bg-red-600 text-white flex justify-between px-[20px] print:hidden">
        <p class="text-[30px] my-auto">List Nota</p>
    </div>
    <div class="h-[16.5cm] p-[10px] print:hidden">
        <p class="my-auto font-bold text-red-600 text-[30px] mb-[10px]">PENDING</p>
        <div class="flex gap-[10px] w-full overflow-auto">
            <div v-for="(data, index) in listPending" :key="index"
                ...
                <div class="mx-auto flex gap-[10px]">
                    ...
                    <button @click="cancel(data.id)"
                        class="bg-red-500 border-none p-[5px] rounded-md w-fit mx-auto text-white mt-[5px]">CANCEL</button>
                </div>
            </div>
        </div>
    </div>
    <a-modal v-model:open="modal" title="Detail Transaksi" class="print:hidden">
        <template #footer>
            <button @click="print" class="px-4 py-2 text-white bg-red-600 rounded-md print:hidden">PRINT</button>
        </template>
    ...
    <div class="hidden print:block">
        ...
        <div v-for="(data, index) in detailNota" :key="index" class="flex justify-between gap-[30px]">
            ...
            <p class="w-[2.5cm] text-end">{{ formatNumber(data.harga) }}</p>
        </div>
        <div class="flex justify-between border-t-[2px] border-dashed border-gray-400">
            <p>Diskon :</p>
            <p class="text-end w-[100px]">{{ diskon }} %</p>
        </div>
        <div class="flex justify-between gap-[30px]">
            <p>Total :</p>
            <p class="text-end w-[100px]">{{ formatNumber(totalPrice) }}</p>
        </div>
    </div>
</template>
<script>
